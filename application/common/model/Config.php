...
class Config extends Model
{
    ...
    public static function getValue($name,$group){
        if(!$name){
            return false;
        }
        $value = Cache::get('config_'.$group.'_'.$name);

        if(!$value || config('app_debug')){
            $value = self::where('name',$name)->where('group',$group)->value('value');
            Cache::set('config_'.$group.'_'.$name,$value,3600);
        }
        return $value;
    }
}
