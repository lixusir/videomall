...
class Auth
{
    ...
    {
        if (empty($mobile) || User::getByMobile($mobile)) {
        ...
        if(isset($extend['invite_code'])){
            ...
            if(isset($extend['invite_code']) && empty($parent)){
                $this->setError('邀请码错误');
                return false;
            }
        }
    ...
    public function direct($user_id)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                $this->_token = Random::uuid();
                Db::name('user_token')->where('user_id',$user->id)->where('expiretime','>=',time())->setField('expiretime',time());
                Token::set($this->_token, $user->id, $this->keeptime);
