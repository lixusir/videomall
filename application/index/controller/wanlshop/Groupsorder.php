 ...
 * @icon fa fa-circle-o
 */
class Groupsorder extends Wanlshop
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\index\model\wanlshop\groups\Order;
    ...
    public function invoice($ids = null)
    {
        ...
        foreach ($row as $data) {
            ...
            $data['address'] = model('app\index\model\wanlshop\groups\OrderAddress')
    ...
    public function delivery($ids = null)
    {
        ...
        foreach ($row as $vo) {
            ...
            $vo['address'] = model('app\index\model\wanlshop\groups\OrderAddress')
	...
	private function pushOrder($order_id = 0, $state = '已发货')
	{
		...
		$orderGoods = model('app\index\model\wanlshop\groups\OrderGoods')
