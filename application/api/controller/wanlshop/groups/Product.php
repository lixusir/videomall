...
class Product extends Api
{
	 ...
	 * 获取商品推荐列表
	 *
	 * @ApiSummary  (WanlShop 产品接口获取商品推荐列表)
	 * @ApiMethod   (GET)
	 * 
	 */
	public function recommend($cid = 0, $shop_id = null)
	{
		...
		// 获取子类目
		if($cid){
			$where['category_id'] = ['in', implode(',', array_column(Tree::instance()->init(model('app\api\model\wanlshop\Category')->all())->getChildren($cid, true), 'id'))];
		}
		// 查询商家
		if($shop_id){
		...
		$where['status'] = 'normal';
		// 商品
		$list = model('app\api\model\wanlshop\groups\Goods')
			->where($where)
			->orderRaw('rand()')
			->paginate();
		foreach ($list as $row) {
    ...
    public function goods()
    {
		...
		$groupsModel = model('app\api\model\wanlshop\groups\Groups');
		// 查询商品
		$goods = $goodsModel
			->where(['id' => $id])
			->field(
	...
	{
	    $this->success('查询成功', model('app\api\model\wanlshop\groups\GoodsSku')->get($sku_id));
	}
